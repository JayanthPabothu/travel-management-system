...
def update_user(user_id):
    ...
    def update_data(user_id, update):
        ...
        if(name1=='' or email1=='' or password1=='' or street1=='' or city1=='' or phone1=='' or DOB_day1=='' or DOB_month1 == '' or DOB_year1 == '' or zipcode1 == ''):
            ...
            cursor1 = con1.cursor()
            cursor1.execute("SELECT EXISTS(SELECT EMAIL_ID FROM SYS_ADMIN WHERE ADMIN_ID<>%s AND EMAIL_ID=%s);", [str(user_id), str(email1)])
    ...
    cursor = con.cursor()
    cursor.execute("SELECT * FROM SYS_ADMIN WHERE ADMIN_ID=%s", [int(user_id)])
