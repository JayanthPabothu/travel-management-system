...
CREATE PROCEDURE UPDATE_ADMIN_DETAILS(IN AD_ID INT, IN NEW_ADMIN_NAME VARCHAR(20), IN NEW_EMAIL_ID VARCHAR(25),
...
CREATE PROCEDURE UPDATE_ROUTE(IN NEW_ROUTE_ID INT, IN FLIGHT_TIME TIME)
BEGIN
	UPDATE ROUTE
    SET
...
CREATE PROCEDURE INSERT_JOURNEY_DETAILS(IN JOURNEYDATE DATE, IN FLIGHT_ID VARCHAR(10), IN STAT INT)
BEGIN
    ...
    VALUES ( JOURNEYDATE, FIRSTCLASS, ECONOMY, BUSINESS, PREMIUM, FLIGHT_ID, STAT);
END $$
...
CREATE PROCEDURE DELETE_JOURNEY(IN ID INT)
BEGIN
	DELETE FROM JOURNEY_FLIGHT
    WHERE JOURNEY_ID = ID;
END $$
